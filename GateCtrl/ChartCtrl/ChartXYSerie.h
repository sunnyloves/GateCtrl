...
..
..
其中共有nColCnt列 nRowCnt行,nLineCnt条曲线nLineCnt=nColCnt-1
pAxeLine[i]里应该是(x,y[i])格式的数据
========================================================*/

typedef CArray<SChartXYPoint,SChartXYPoint&> DataArray;

//曲线数据结构 第一位是曲线名,从文件解析出来,之后都是数据点
typedef struct _DataLine 
{
	CString		m_sLineName;
	DataArray	m_dbLineDataArray;
}DataLine;
...
public:
	...
	*    修改:    
	*----------------------------------------------------------------------------*/ 
	void SetPointsArray(DataArray& DA);
