...
*    函数名:	SetPointsArray
*    功能:    添加自定义格式数组
*    BY:    7/31/2010 Kevin Sun
*    输入参数:    
*    返回参数:    
*    备注:    
*    修改:    
*----------------------------------------------------------------------------*/ 
void CChartXYSerie::SetPointsArray(DataArray& DA)
{
	UINT m_nPointsCnt = DA.GetCount();
	SChartXYPoint* pPoints = new SChartXYPoint[m_nPointsCnt];	

	for (UINT i=0;i<m_nPointsCnt;i++)
	{
		pPoints[i].X = DA.GetAt(i).GetX();
		pPoints[i].Y = DA.GetAt(i).GetY();
	}

	CChartSerieBase<SChartXYPoint>::SetPoints(pPoints,m_nPointsCnt);
